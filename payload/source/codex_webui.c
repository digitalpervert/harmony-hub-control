...
static void page_end(FILE *f) {
    fputs(
        ...
        "function prontoEntries(t){const rows=[],lines=String(t||'').replace(/\\r/g,'').split('\\n'),re=/((?:0000|0100|5000|6000|7000)(?:\\s+[0-9a-fA-F]{4}){10,})/g;lines.forEach((line,i)=>{let m;while((m=re.exec(line))){const hex=m[1].replace(/\\s+/g,' ').trim(),raw=prontoToHarmonyRaw(hex),name=safeImportName(line.slice(0,m.index).replace(/[:=,]+$/,'').trim()||'Pronto '+(i+1));rows.push({name:name,meta:raw?'Pronto Hex converted timing':'Pronto Hex unsupported',raw:raw});}});return rows;}"
        "function codeStringRow(name,s,meta){s=String(s||'').trim();if(!s)return null;if(/^G:[^\\r\\n]+?:\\d+$/i.test(s))return{name:name,meta:meta||'Harmony compact code',keycode:s};if(/^F[0-9A-F]+(?:[PS][0-9A-F]+)+$/i.test(s))return{name:name,meta:meta||'Harmony raw timing',raw:s};let raw=sendirRaw(s);if(raw)return{name:name,meta:meta||'Global Cache sendir converted timing',raw:raw};raw=prontoToHarmonyRaw((s.match(/(?:0000|0100|5000|6000|7000)(?:\\s+[0-9a-fA-F]{4}){10,}/)||[])[0]||'');if(raw)return{name:name,meta:meta||'Pronto Hex converted timing',raw:raw};raw=broadlinkRaw(s);if(raw)return{name:name,meta:meta||'BroadLink base64 converted timing',raw:raw};const rr=rawTimingRows(s,name);return rr[0]||null;}"
        "function lircProp(block,key){const m=String(block||'').match(new RegExp('\\\\b'+key+'\\\\s+([^\\\\n]+)','i'));return m?m[1].trim():'';}function lircNums(block,key){return lircProp(block,key).split(/\\s+/).map(Number).filter(Number.isFinite);}"
        "function lircCodeRaw(block,hex){const bits=parseInt(lircProp(block,'bits'),10)||32,one=lircNums(block,'one'),zero=lircNums(block,'zero'),head=lircNums(block,'header'),ptrail=parseInt(lircProp(block,'ptrail'),10)||0,gap=parseInt(lircProp(block,'gap'),10)||45000,freq=parseInt(lircProp(block,'frequency'),10)||38000;if(one.length<2||zero.length<2)return'';let code=parseInt(String(hex||'').replace(/^0x/i,''),16);if(!Number.isFinite(code))return'';const seq=[];if(head.length>=2){pulse(seq,1,head[0]);pulse(seq,0,head[1]);}const arr=/REVERSE/i.test(lircProp(block,'flags'))?lsbBits(code,bits):msbBits(code,bits);arr.forEach(b=>{pulse(seq,1,b?one[0]:zero[0]);pulse(seq,0,b?one[1]:zero[1]);});if(ptrail)pulse(seq,1,ptrail);pulse(seq,0,gap);return seqRaw(freq,seq);}"
        "function lircEntries(t){const out=[];String(t||'').replace(/\\r/g,'').split(/begin\\s+remote/i).slice(1).forEach((part,bi)=>{const block=(part.split(/end\\s+remote/i)[0]||''),remote=safeImportName(lircProp(block,'name')||'LIRC remote '+(bi+1)),freq=parseInt(lircProp(block,'frequency'),10)||38000,rawSec=(block.match(/begin\\s+raw_codes([\\s\\S]*?)end\\s+raw_codes/i)||[])[1];if(rawSec){let name='',vals=[];function push(){const raw=harmonyRawFromTimings(freq,vals);if(name&&raw)out.push({name:name,meta:'LIRC raw timings '+freq+' Hz ('+vals.length+' durations)',raw:raw});vals=[];}rawSec.split('\\n').forEach(line=>{line=line.trim();if(!line)return;const m=line.match(/^name\\s+(.+)/i);if(m){push();name=safeImportName(m[1]);}else vals=vals.concat(line.split(/\\s+/).map(Number).filter(Number.isFinite));});push();}const codes=(block.match(/begin\\s+codes([\\s\\S]*?)end\\s+codes/i)||[])[1];if(codes){codes.split('\\n').forEach(line=>{line=line.trim();const m=line.match(/^(\\S+)\\s+(0x[0-9a-fA-F]+|[0-9a-fA-F]+)\\b/);if(!m)return;const raw=lircCodeRaw(block,m[2]);out.push({name:safeImportName(m[1]),meta:raw?'LIRC '+remote+' rendered timing':'LIRC code unsupported',raw:raw});});}});return out;}"
        "function girrEntries(t){const out=[];try{const doc=new DOMParser().parseFromString(String(t||''),'application/xml');if(doc.querySelector('parsererror'))return[];doc.querySelectorAll('command').forEach((cmd,i)=>{const name=safeImportName(cmd.getAttribute('name')||cmd.getAttribute('id')||'GIRR '+(i+1));['raw','pronto','ccf','sendir'].forEach(tag=>{cmd.querySelectorAll(tag).forEach(el=>{const txt=(el.textContent||'').trim();let row=null;if(tag==='raw')row=rawTimingRows(txt,name)[0];else row=codeStringRow(name,txt,tag==='sendir'?'Global Cache sendir converted timing':'GIRR '+tag+' converted timing');if(row)out.push(row);});});});}catch(e){}return out;}"
        ...
        "async function loadIrdSource(src){if(irdbCache[src])return irdbCache[src];if(src==='flipper'){const j=await (await fetch(FLIPPER_INDEX)).json();const files=j.tree||j.files||[];irdbCache[src]=files.filter(x=>!x.type||x.type==='blob').map(x=>(x.path||x.name||x).replace(/^\\//,'')).filter(x=>x.endsWith('.ir')).map(path=>({source:src,path:path}));}else if(src==='lirc'){const j=await (await fetch(LIRC_INDEX)).json();const files=j.tree||[];irdbCache[src]=files.filter(x=>x.type==='blob').map(x=>(x.path||'').replace(/^\\//,'')).filter(x=>x&&x!=='README.md'&&!/\\.(png|jpg|jpeg|gif|md|html)$/i.test(x)).map(path=>({source:src,path:path}));}else if(src==='smartir'){const j=await (await fetch(SMARTIR_INDEX)).json();const files=j.tree||[];irdbCache[src]=files.filter(x=>x.type==='blob').map(x=>(x.path||'').replace(/^\\//,'')).filter(x=>/^codes\\/.+\\.json$/i.test(x)).map(path=>({source:src,path:path}));}else{const t=await (await fetch(IRDB_BASE+'index')).text();irdbCache[src]=t.replace(/\\r/g,'').split('\\n').map(x=>x.trim()).filter(x=>x.endsWith('.csv')).map(path=>({source:src,path:path}));}return irdbCache[src];}"
        "async function loadIrdIndexes(){const src=$('irdbSource')?.value||'all',want=src==='all'?['irdb','flipper','lirc','smartir']:(src==='remotecentral'||src==='custom'?[]:[src]);const lists=await Promise.all(want.map(loadIrdSource));irdbIndex=lists.flat();return irdbIndex;}"
...
static void wifi_form(FILE *f, const struct wifi_config *cfg) {
    ...
    fprintf(f, "<label>Wi-Fi network name (SSID)</label><input name='ssid' required value='"); html(f, cfg->ssid); fprintf(f, "'><div class='help'>The hub will join this network after reboot.</div>");
...
static void backup_panel(FILE *f) {
    ...
    fprintf(f, "<label>Backup type</label><select name='target'>");
    ...
    fprintf(f, "</select>");
    fprintf(f, "<label>Backup file</label><input id='importFile' type='file'>");
    fprintf(f, "<label>Backup contents</label><textarea class='textarea-tall' name='payload' spellcheck='false' required></textarea>");
...
static void system_panel(FILE *f) {
    ...
    fprintf(f, "</pre></details></div><div class='panel' style='margin-top:12px'><h3>Software Update</h3><div class='help'>Check GitHub for newer files, copy them to the hub, verify checksums, and restart the local services. SSH access is not changed. The default public repository works without a token. Use the token field only for private repositories.</div><div class='grid two'><div><label>Optional public mirror URL</label><input id='updateRepo' value='https://raw.githubusercontent.com/Ripthulhu/harmony-hub-control/main/payload/bin/'></div><div><label>GitHub token (private repos only)</label><input id='updateToken' type='password' autocomplete='off' placeholder='optional; used only by this browser'></div></div><div class='actions'><button id='updateCheck' type='button' class='secondary'>Check For Updates</button><button id='updateInstall' type='button'>Install Update</button><button id='updateRefresh' type='button' class='secondary'>Show Installed Versions</button></div><pre id='updateLog' class='mini'>Ready. Check the public repo, or paste a token if the repo is private.</pre></div><div class='panel' style='margin-top:12px'><div class='help'>Refresh Home Assistant discovery if new devices or commands do not appear after changes.</div><form method='post' action='/system#system'><div class='actions'><button name='action' value='rediscover' type='submit'>Refresh Home Assistant Discovery</button><button name='action' value='reboot' type='submit' class='secondary'>Reboot Hub</button></div></form></div></section>");
}
