...
static void page_end(FILE *f) {
    fputs(
        ...
        "const UPDATE_DEFAULT_RAW='https://raw.githubusercontent.com/Ripthulhu/harmony-hub-control/main/payload/bin/';function updateBase(){let b=($('updateRepo')?.value||'').trim()||UPDATE_DEFAULT_RAW;return b.endsWith('/')?b:b+'/';}function updateToken(){return($('updateToken')?.value||'').trim();}"
...
static void system_panel(FILE *f) {
    ...
    fprintf(f, "</pre></details></div><div class='panel' style='margin-top:12px'><h3>Software Update</h3><div class='help'>Pull the latest control binaries from GitHub, stage them on the hub, verify them against MANIFEST.txt, then restart the local services. Dropbear/SSH is left untouched. The default public repository works without a token. Add a GitHub token only when using a private repo or fork.</div><div class='grid two'><div><label>Raw base URL</label><input id='updateRepo' value='https://raw.githubusercontent.com/Ripthulhu/harmony-hub-control/main/payload/bin/'></div><div><label>GitHub token</label><input id='updateToken' type='password' autocomplete='off' placeholder='optional; used only by this browser for private repos'></div></div><div class='actions'><button id='updateCheck' type='button' class='secondary'>Check Repo</button><button id='updateInstall' type='button'>Install Update</button><button id='updateRefresh' type='button' class='secondary'>Refresh Local Status</button></div><pre id='updateLog' class='mini'>Ready. Check the public repo, or paste a token for a private repo.</pre></div><div class='panel' style='margin-top:12px'><div class='help'>Reload discovery if Home Assistant does not show new entities after device or command changes.</div><form method='post' action='/system#system'><div class='actions'><button name='action' value='rediscover' type='submit'>Reload MQTT Discovery</button><button name='action' value='reboot' type='submit' class='secondary'>Reboot Hub</button></div></form></div></section>");
}
