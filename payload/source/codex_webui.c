...
static void page_head(FILE *f, const char *title) {
    ...
    fputs(
        ...
        ".grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px}.cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px}.dashboard-cards{grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:12px}.panel,.stat,.setup-shell{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:16px;box-shadow:0 1px 2px rgba(20,40,32,.04);min-width:0}.stat{min-height:92px;padding:17px}.stat .value{font-size:20px;font-weight:700;margin-top:5px}.stat .label{text-transform:uppercase;letter-spacing:0;color:var(--muted);font-size:11px}"
...
static void ir_render_unmapped_commands(FILE *f, const struct ir_device *dev) {
    ...
    fprintf(f, "<div class='ir-unmapped'><div class='muted mini'>Remote skin maps %d of %d saved commands.</div>", mapped, dev->command_count);
    if (unmapped > 0) {
        fprintf(f, "<h3>Saved buttons not on this remote</h3><div class='help'>These commands are saved for the device, but the photo does not have a matching physical button. Send them from here.</div><div class='ir-quick-grid'>");
        for (i = 0; i < dev->command_count; i++) {
...
static void ir_render_remote_skin(FILE *f, const struct ir_device *dev) {
    ...
    fprintf(f, "</div></div><div class='help'>Highlighted areas send the closest saved command for this device. Buttons without a saved match stay inactive.</div>");
...
static void ir_setup_flow(FILE *f, const struct ir_inventory *inv) {
    ...
    fprintf(f, "<div class='wizard-panel active' data-panel='device'><div class='callout'><strong>Start with the device profile.</strong>Enter the real manufacturer and model. Search uses these details on the next step, and saving first keeps this device selected while you import commands.</div><form id='profileDeviceForm' method='post' action='/ir/new-device#ir'>");
