...
static void page_end(FILE *f) {
    fputs(
        ...
        "function csvProtocolRaw(proto,d,s,f,name=''){proto=String(proto||'');const D=Number(d),F=Number(f),S=String(s===undefined?'':s).trim();if(!Number.isFinite(D)||!Number.isFinite(F)||D<0||F<0)return'';if(/^RC5X?/i.test(proto))return rc5Raw({address:D.toString(16),command:F.toString(16),toggle:'0'});if(/^RC6/i.test(proto))return rc6Raw({address:D.toString(16),command:F.toString(16),toggle:'0'});if(/^MCE$/i.test(proto))return mceRaw(D,S,F);if(/^RECS80$/i.test(proto))return recs80Raw(D,S,F,name);if(/^Akai$/i.test(proto))return akaiRaw(D,F);if(/^Denon(?:\\{[12]\\})?$/i.test(proto))return denonRaw(D,F);if(/^Sharp(?:\\{[12]\\})?$/i.test(proto))return sharpRaw(D,F);if(/^DirecTV$/i.test(proto))return directvRaw(D,F);if(/^G\\.I\\.Cable(?:\\{[12]\\})?$/i.test(proto))return giCableRaw(D,F);if(/^Proton$/i.test(proto))return protonRaw(D,F);if(/^F12$/i.test(proto))return f12Raw(D,S,F);if(/^JVC$/i.test(proto))return jvcRaw(D,F);if(/^Panasonic$/i.test(proto))return panasonicRaw(D,S,F);if(/^Aiwa$/i.test(proto))return aiwaRaw(D,S,F);if(/^Panasonic_Old$/i.test(proto))return panasonicOldRaw(D,S,F);if(/^Dish_Network$/i.test(proto))return dishRaw(D,S,F);if(/^48-NEC1$/i.test(proto))return nec48Raw(D,S,F,0);if(/^Blaupunkt$/i.test(proto))return blaupunktRaw(D,S,F);if(/^RCA(?:-38)?(?:\\(Old\\))?$/i.test(proto))return rcaRaw(proto,D,F);if(/^Sony(12|15|20)?/i.test(proto)){const bits=(proto.match(/Sony(\\d+)/i)||[])[1]||'12';let addr=D;if(bits==='20'&&S&&S!=='-1'){const sub=Number(S);if(Number.isFinite(sub)&&sub>=0)addr=(D&31)|((sub&255)<<5);}return sircRaw({address:addr.toString(16),command:F.toString(16)},'SIRC'+bits);}return'';}"
