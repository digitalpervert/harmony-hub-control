...
static void ir_lab_panel(FILE *f) {
    struct ir_inventory *inv = (struct ir_inventory *)calloc(1, sizeof(*inv));
    if (!inv) return;
    fprintf(f, "<section id='view-lab' data-view='lab' class='section'><div class='section-head'><div><h2>IR Sweep</h2><div class='section-lead'>Search IR libraries, queue matching commands, and send paced sequences. The hub stores selected commands; the browser handles large database searches.</div></div><span class='pill'>up to %d saved commands/device</span></div>", MAX_IR_STORED_COMMANDS);
    if (load_ir_inventory(inv) != 0) {
        fprintf(f, "<div class='panel'><pre>Unable to read ");
        html(f, DEVICE_LIST);
        fprintf(f, "</pre></div></section>");
    ...
    fprintf(f, "<div class='lab-layout'><div class='panel'><h3>Library Search</h3><div class='callout'><strong>Search first, send second.</strong>Find a real device file or path, load only matching commands into the queue, then save and send from the queue panel. Search logs show source counts, unsupported protocols, duplicate paths, and duplicate IR payloads.</div>");
    fprintf(f, "<div class='lab-toolbar'><div><label>Save/send target</label><select id='labDevice'><option value='__auto_lab__' selected>Temporary sweep device</option>");
    ir_device_options(f, inv, NULL);
    ...
    fprintf(f, "<div class='actions'><button id='labCandidatesBtn' type='button' class='secondary'>Search Devices</button><button id='labScan' type='button'>Load Commands</button><button id='labStored' type='button' class='secondary'>Use Saved Commands</button><button id='labClear' type='button' class='ghost'>Clear Queue</button></div>");
    ...
    fprintf(f, "<div class='panel'><h3>Send Queue</h3><div class='help'>Checked commands are sent in short cancellable chunks. Duplicate IR payloads are skipped even when databases give them different names. Sending saves queued commands first. Stop interrupts the active hub-side batch instead of waiting for a long chunk to drain.</div><div class='lab-summary'><span id='labSummary'>0 queued</span><span>batch limit %d</span></div><div class='queue-tools'><button id='labSelectAll' type='button' class='ghost'>Select all</button><button id='labSelectNone' type='button' class='ghost'>Select none</button><button id='labDropUnchecked' type='button' class='ghost'>Drop unchecked</button></div><div id='labQueue' class='queue-list'><div class='muted mini'>No commands queued.</div></div>", MAX_IR_BATCH_COMMANDS);
